...
class task {
 ...
 public:
  ...
  // generate some experimental queries
  int generated_query() {
    // FILE *topo_f;
    // topof = fopen(file_name, 'w');
    int u = 1000;
    int v = 1200;
    int cost_limit = 1000;
    vector<tuple<int, int, int, int>> task_l;
    int i = 0;
    for (i=0; i<2; i++){
        // fprintf(topo_f, "%d %d %d %d %d\n", &u, &v, &cost_limit, &cost_limit, &cost_limit);
        // cost_limit += 1000*(i+1);
        source_l_.push_back(u);
        sink_l_.push_back(v);
        delay_l_.push_back(cost_limit);
  ...
  int load_query(const char *file_name) {
