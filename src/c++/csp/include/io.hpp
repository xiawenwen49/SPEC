...
int read_graph(const char *file_name, Graph_t *g, int graph_type) {
  char temp[255];
  FILE *topo_f;
  int id, src, snk;
  int weight, delay;
  ...
  int ret;
  if (graph_type == 1) {
    ret = fscanf(topo_f, "%d", &num_of_vertex);
    ret = fscanf(topo_f, "%c", temp);
    ret = fscanf(topo_f, "%d", &num_of_edge);
    ret = fscanf(topo_f, "%c", temp);
    num_of_edge *= 2;
    g->v_l.clear();
...
//      snk--;
    ...
    for (; fscanf(topo_f, "%d:", &u) == 1;) {
      for (; fscanf(topo_f, "%d", &v) == 1;) {
        ...
        g->e_l.back()->assign(M, u, v, w, c); // id, src, snk, len, delay
